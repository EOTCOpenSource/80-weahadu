"""Merge legacy/am/*.json into the single legacy/minified/80-weahadu.json bundle.
...
def merge_json_files():
    """
    Merges all JSON files from legacy/am/ into a single file
    at legacy/minified/80-weahadu.json.
    """
    output_data = []

    # Anchored at the repo root so the script works from any directory.
    json_files = sorted(glob.glob(repo("legacy", "am", "*.json")))
    ...
    os.makedirs(repo("legacy", "minified"), exist_ok=True)

    # Write the combined data to the output file
    output_file = repo("legacy", "minified", "80-weahadu.json")
    try:
